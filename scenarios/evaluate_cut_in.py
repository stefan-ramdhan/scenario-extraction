#!/usr/bin/env python3
"""
evaluate_cut_in.py — Compare cut-in scenario extractions against ground-truth
cut-in APs, split by phase.

GT APs (from extract_aps.py):
...
Extraction variants → GT AP mapping:
...
Three evaluation strategies (select with --strategy):
...
Track resolution
----------------
- The vehicle of interest is keyed as 'yield_vehicle_1' in entity_mapping,
  with a fallback to the first value if that key is absent.
- 'track_car_X' / 'track_truck_X' / 'track_bus_X' → integer X is the
  RS2V track_id; the AV2 UUID is found via *_matched.json.
- Extractions whose track cannot be resolved are counted as False Positives.
- GT timestamps are start_global_time_s / end_global_time_s (absolute seconds).
- Extraction timestamps (start_frame / end_frame) are nanoseconds.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Variant → GT AP mapping (exact names — no prefix matching)
#
# Output is shown in GT_AP_ORDER so phases appear in the natural sequence.
# ─────────────────────────────────────────────────────────────────────────────
...
def resolve_gt_ap(variant: str) -> str | None:
    """Return the GT AP name for a variant, or None if not in the mapping."""
    return VARIANT_GT_AP.get(variant)
...
def _iou(a_start: float, a_end: float,
    ...
    """Return IoU of two intervals, in [0, 1]."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate cut-in extractions against GT APs by phase.")
    parser.add_argument(
    ...
    # Discover cut-in extraction variants present in my_results (exact name match)
    variants: set[str] = set()
    for uuid8 in uuids:
        scenario_dir = MY_RESULTS_DIR / uuid8
        if scenario_dir.exists():
            for d in scenario_dir.iterdir():
                if d.is_dir() and d.name in VARIANT_GT_AP:
                    variants.add(d.name)

    if not variants:
        print("No cut-in extraction variants found under my_results/.")
        return

    # Group variants by GT AP, in phase order
    groups: dict[str, list[str]] = defaultdict(list)
    for v in sorted(variants):
        groups[resolve_gt_ap(v)].append(v)

    ordered_ap_names = [ap for ap in GT_AP_ORDER if ap in groups]

    print(f"UUIDs with ground truth : {len(uuids)}")
    print(f"GT AP groups (in order) : {ordered_ap_names}")
    ...
    grand_counts: list[tuple] = []

    first_group = True
    for ap_name in ordered_ap_names:
        group_variants = groups[ap_name]
        if not first_group:
            print()
        first_group = False
