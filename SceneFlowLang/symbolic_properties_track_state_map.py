...
def lateral_velocity_leftward(ego, entity):
    def _check(ego_nodes, v_nodes):
        ...
        return lat != 0 and (lat * lat_vel < 0) and abs(lat_vel) >= 0.25
    return partial(_check, ego, entity)

def ego_in_intersection(ego):
    """True when ego has an isIn edge to the Intersection node."""
    intersection_nodes = partial(P.filterByAttr, "G", "name", "Intersection")
    return non_empty(partial(P.intersection, entity_lanes(ego), intersection_nodes))

def lateral_offset_right_at_least(ego, entity, min_offset=1.0):
    """True when entity is to the right of ego by at least min_offset metres (lat <= -min_offset)."""
    def _check(ego_nodes, v_nodes):
        ego_yaw = P.getAttr(ego_nodes, "yaw")
        ex      = P.getAttr(ego_nodes, "position_x")
        ey      = P.getAttr(ego_nodes, "position_y")
        px      = P.getAttr(v_nodes,   "position_x")
        py      = P.getAttr(v_nodes,   "position_y")
        if any(v is None for v in [ego_yaw, ex, ey, px, py]):
            return False
        lat = -math.sin(ego_yaw) * (px - ex) + math.cos(ego_yaw) * (py - ey)
        return lat <= -min_offset
    return partial(_check, ego, entity)

def in_lateral_corridor_right(ego, entity, min_offset=1.0, max_offset=2.5):
    """
    True when entity is to the right of ego within [min_offset, max_offset] metres.
    Rightward lateral = negative value in ego frame.
    Defaults match extract_aps.py: min_lat_offset=1.0, corridor_max=2.5.
    """
    def _check(ego_nodes, v_nodes):
        ego_yaw = P.getAttr(ego_nodes, "yaw")
        ex      = P.getAttr(ego_nodes, "position_x")
        ...
        if any(v is None for v in [ego_yaw, ex, ey, px, py]):
            return False
        lat = -math.sin(ego_yaw) * (px - ex) + math.cos(ego_yaw) * (py - ey)
        return -max_offset <= lat <= -min_offset
    return partial(_check, ego, entity)
...
def veh_in_adj_right_lane(ego, vehicle):
    in_lane1 = veh_in_right_lane(vehicle)
    # When ego is in an intersection, MAP SGs may reassign the cutting-in vehicle
    # from Lane_1 to Lane_0 before it has fully merged.  Mirror extract_aps.py's
    # intersection fallback: use a lateral band [-7.0, -1.5] m instead of lane
    # membership when that happens.
    lateral_fallback = in_lateral_corridor_right(ego, vehicle, min_offset=1.5, max_offset=7.0)
    in_right_lane_or_fb = partial(P.logic_or, in_lane1,
                                   partial(P.logic_and, ego_in_intersection(ego), lateral_fallback))
    return partial(P.logic_and,
...
def veh_straddling_ego_lane(ego, vehicle):
    # Non-intersection: vehicle in ego lane (Lane_0) with rightward offset >= 1.0 m.
    # Mirrors extract_aps: in_ego_lane AND NOT is_intersect AND abs(lat) >= 1.0.
    non_intersect_ok = partial(P.logic_and,
                               veh_in_middle_lane(vehicle),
                               partial(P.logic_and,
                                       partial(P.logic_not, ego_in_intersection(ego)),
                                       lateral_offset_right_at_least(ego, vehicle, 1.0)))
    # Intersection / not-in-ego-lane fallback: corridor [1.0, 2.5] m.
    # Mirrors extract_aps: NOT(in_ego_lane AND NOT is_intersect) → corridor.
    in_corridor = in_lateral_corridor_right(ego, vehicle, min_offset=1.0, max_offset=2.5)
    return partial(P.logic_and,
                   partial(P.logic_or, non_intersect_ok, in_corridor),
                   partial(P.logic_and,
                           is_too_close(ego, vehicle, "visible"),
...
def lead_veh_too_close(ego, vehicle, dist):
    # drop lane_ok: MAP SGs assign post-merge vehicles to Lane_-1/Intersection rather than
    # Lane_0, suppressing p3 entirely.  direct_behind (atDRearOf + inDFrontOf edges) already
    # guarantees |lat| < ~1 m, matching extract_aps p3's sole geometric check.
    return partial(P.logic_and,
...
def cut_in_from_right_veh_decel(decel=3.0):
    return SymbolicProperty(
        "cut_in_from_right_veh_decel",
        "p1 & (p1 U (p2 & (p2 U (p3 & X(p3 U (p3 & X ~p3)))))) & F (p1 & (p1 U (p2 & (p2 U (p3 & X(p3 U (p3 & X ~p3)))))) & veh_decel)",
        [("p1",        veh_in_adj_right_lane(EGO, YIELD_VEHICLE1)),
         ("p2",        veh_straddling_ego_lane(EGO, YIELD_VEHICLE1)),
         ("p3",        lead_veh_too_close(EGO, YIELD_VEHICLE1, "very_near")),
         ("veh_decel", long_deceleration_exceeds(YIELD_VEHICLE1, decel))],
...
def cut_in_from_right_weather():

    prop = SymbolicProperty(
        "cut_in_from_right_snowy",
        "p1 & (p1 U (p2 & (p2 U (p3 & X(p3 U (p3 & X ~p3)))))) & weather", # this gets us from start to end. (start being beginning of the cutin, (last frame of p1).)
        [("p1", veh_in_adj_right_lane(EGO, YIELD_VEHICLE1)),
         ("p2", veh_straddling_ego_lane(EGO, YIELD_VEHICLE1)),
         ("p3", lead_veh_too_close(EGO, YIELD_VEHICLE1, "very_near")),
         ("weather", weather_checking("snow/frosty"))],
    [YIELD_VEHICLE1])
    return prop
