...
def nearest_same_lane(vehicle1, vehicle2, switch_frames=1, hold_frames=0):
    ...
    def _raw_best_name(lane_tiers, ego_node):
        ...
        if ego_node is None:
            return next(iter(best_tier)).name
        ex = ego_node.attr.get("location_x", None)
        ey = ego_node.attr.get("location_y", None)
        if ex is None or ey is None:
            return next(iter(best_tier)).name
        def _sq_dist(n):
            nx = n.attr.get("location_x")
            ny = n.attr.get("location_y")
...
def is_behind(vehicle1, vehicle2, longitudinal_threshold=0.0,
                     lateral_tolerance=None):
    # In RS2V BEV coords: lower location_y = further ahead of ego.
    # v1 is behind v2 iff v1.y > v2.y, i.e. (v2.y - v1.y) < longitudinal_threshold.
    def _behind_check(v1_nodes, v2_nodes, long_thresh, lat_tol):
        y1 = P.getAttr(v1_nodes, "location_y")
        y2 = P.getAttr(v2_nodes, "location_y")

        if y1 is None or y2 is None:
            return False

        behind_check = bool((y2 - y1) < long_thresh)

        if lat_tol is not None:
            x1 = P.getAttr(v1_nodes, "location_x")
            x2 = P.getAttr(v2_nodes, "location_x")
            if x1 is None or x2 is None:
                return behind_check
            return behind_check and bool(abs(x1 - x2) <= lat_tol)

        return behind_check

    return partial(_behind_check, vehicle1, vehicle2,
                   longitudinal_threshold, lateral_tolerance)
...
def long_following_abstract_debounce(follow_distance, follow_duration, end_hold_frames=2,
    ...
    prop = SymbolicProperty(
        ...
        f"$[{follow_duration}][within_distance & same_lane & behind] & "
        f"((within_distance & same_lane & behind) "
        f"U ((within_distance & same_lane & behind) "
        f"& X($[{end_hold_frames}][~(within_distance & same_lane & behind)])))",
        [("within_distance", debounced_is_too_close(EGO, YIELD_VEHICLE1, follow_distance,
         ...
         ("behind",          is_behind(EGO, YIELD_VEHICLE1)),],
        [YIELD_VEHICLE1])
    return prop
