...
from SG_Primitives import filterByGraphAttr
...
_RSS_RHO         = 1.0   # reaction time (s)
_RSS_A_MAX_ACCEL = 3.5   # max acceleration during reaction phase (m/s^2)
_RSS_A_MIN_BRAKE = 4.0   # minimum braking capability of rear vehicle (m/s^2)
_RSS_A_MAX_BRAKE = 8.0   # maximum braking capability of front vehicle (m/s^2)

_BUS_HALF_LENGTH = 6.0   # half of ~12 m bus
_CAR_HALF_LENGTH = 2.0   # half of ~4 m car / ego


def _half_length(node_set):
    if not isinstance(node_set, set) or len(node_set) != 1:
        return _CAR_HALF_LENGTH
    node = next(iter(node_set))
    base = getattr(node, "base_class", None) or node.attr.get("base_class", "")
    return _BUS_HALF_LENGTH if base == "bus" else _CAR_HALF_LENGTH


def _compute_rss_d_min(v_r, v_f):
    """RSS minimum safe following distance (m). Applies [x]_+ = max(x, 0)."""
    raw = (v_r * _RSS_RHO
           + 0.5 * _RSS_A_MAX_ACCEL * _RSS_RHO ** 2
           + (v_r + _RSS_RHO * _RSS_A_MAX_ACCEL) ** 2 / (2.0 * _RSS_A_MIN_BRAKE)
           - v_f ** 2 / (2.0 * _RSS_A_MAX_BRAKE))
    return max(0.0, raw)


def d_min_violated(ego, vehicle):
    """True when the bumper-to-bumper longitudinal gap is less than d_min.

    Gap = signed projection of centre-to-centre vector onto lead vehicle's
    heading (negative when ego is behind), converted to a positive gap by
    negation, then adjusted to bumper-to-bumper by subtracting each vehicle's
    half-length (2 m for cars/ego, 6 m for buses).

    Returns False if any required attribute is missing (safe default).
    """
    def _check(ego_nodes, veh_nodes):
        x_ego   = P.getAttr(ego_nodes, "position_x")
        y_ego   = P.getAttr(ego_nodes, "position_y")
        x_veh   = P.getAttr(veh_nodes, "position_x")
        y_veh   = P.getAttr(veh_nodes, "position_y")
        yaw_veh = P.getAttr(veh_nodes, "yaw")
        v_r     = P.getAttr(ego_nodes, "velocity")
        v_f     = P.getAttr(veh_nodes, "velocity")

        if any(v is None for v in [x_ego, y_ego, x_veh, y_veh, yaw_veh, v_r, v_f]):
            return False
        ...
        hx = math.cos(yaw_veh)
        hy = math.sin(yaw_veh)
        # negative when ego is behind veh; negate to get positive gap
        long_gap = -(hx * dx + hy * dy) - _half_length(ego_nodes) - _half_length(veh_nodes)

        if long_gap <= 0:
            return False  # overlapping or ego not actually behind — undefined

        return long_gap < _compute_rss_d_min(max(v_r, 0.0), max(v_f, 0.0))

    return partial(_check, ego, vehicle)
...
def long_following_logical_veh_decel(follow_distance, decel, follow_duration=3, end_hold_frames=2,
                                     same_lane_switch_frames=2, same_lane_hold_frames=1,
                                     within_dist_hold_frames=1):
    prop = SymbolicProperty(
        ...
        f"U ((within_distance & same_lane & behind & is_moving_ego) "
        f"& X($[{end_hold_frames}][~(within_distance & same_lane & behind & is_moving_ego)]))) "
        f"& F ((within_distance & same_lane & behind & is_moving_ego) & veh_decel)",
        [("within_distance", debounced_is_too_close(EGO, YIELD_VEHICLE1, follow_distance,
                                                    hold_frames=within_dist_hold_frames)),
         ("same_lane",       nearest_same_lane(YIELD_VEHICLE1, EGO,
                                               switch_frames=same_lane_switch_frames,
...
def long_following_rss_violated(same_lane_switch_frames=2, same_lane_hold_frames=1,
                                within_dist_hold_frames=1):
    """Longitudinal following scenario where the RSS minimum distance is violated.

    P = within_distance(ego, veh1, ~60 m via 'visible' SG relation, bumper-to-bumper)
      & same_lane(ego, veh1)
      & behind(ego, veh1)
      & is_moving(ego)

    V = d_min_violated(ego, veh1)

    LTL: P & (P U (P & V)) & (P U (P & X(~P)))

    'visible' is used for within_distance because it is the SG relation whose
    bumper-to-bumper threshold matches the intended 60 m proximity gate.
    """
    P = "within_distance & same_lane & behind & is_moving_ego"
    V = "d_min_violated"
    ltl = (
        f"({P}) & "
        f"(({P}) U (({P}) & ({V}))) & "
    ...
    prop = SymbolicProperty(
        "long_following_rss_violated",
        ltl,
        [("within_distance", debounced_is_too_close(EGO, YIELD_VEHICLE1, 'visible',
                                                    hold_frames=within_dist_hold_frames)),
         ("same_lane",        nearest_same_lane(YIELD_VEHICLE1, EGO,
                                                switch_frames=same_lane_switch_frames,
                                                hold_frames=same_lane_hold_frames)),
         ("behind",           behind_kinematic(EGO, YIELD_VEHICLE1)),
         ("is_moving_ego",    is_moving_video(EGO)),
         ("d_min_violated",   d_min_violated(EGO, YIELD_VEHICLE1))],
        [YIELD_VEHICLE1])
    return prop

def long_following_rss_violation_interval(same_lane_switch_frames=2, same_lane_hold_frames=1,
                                          within_dist_hold_frames=1):
    """Extracts the contiguous intervals during which the RSS d_min is violated.

    Q = P & V
      = (within_distance & same_lane & behind & is_moving_ego) & d_min_violated

    LTL: Q & (Q U (Q & X(~Q)))

    Accepts exactly the maximal contiguous blocks where d_min is violated inside
    a longitudinal-following scenario. A single-frame violation is also captured
    (the Until is satisfied immediately when X(~Q) holds at the first frame).
    """
    Q = "within_distance & same_lane & behind & is_moving_ego & d_min_violated"
    ltl = f"({Q}) & (({Q}) U (({Q}) & X(~({Q}))))"
    prop = SymbolicProperty(
        "long_following_rss_violation_interval",
        ltl,
        [("within_distance", debounced_is_too_close(EGO, YIELD_VEHICLE1, 'visible',
                                                    hold_frames=within_dist_hold_frames)),
         ("same_lane",        nearest_same_lane(YIELD_VEHICLE1, EGO,
...
def road_surface_checking(road_surface_condition):
    return partial(filterByGraphAttr, "road_surface", road_surface_condition)
...
def long_following_rss_violation_interval_weather(weather_condition,
                                                   same_lane_switch_frames=2,
                                                   same_lane_hold_frames=1,
                                                   within_dist_hold_frames=1):
    """RSS d_min violation interval property filtered to a specific weather condition."""
    Q = "within_distance & same_lane & behind & is_moving_ego & d_min_violated"
    ltl = f"({Q}) & (({Q}) U (({Q}) & X(~({Q})))) & weather"
    prop = SymbolicProperty(
        f"long_following_rss_violation_interval_{weather_condition}",
        ltl,
        [("within_distance", debounced_is_too_close(EGO, YIELD_VEHICLE1, 'visible',
                                                     hold_frames=within_dist_hold_frames)),
         ("same_lane",        nearest_same_lane(YIELD_VEHICLE1, EGO,
         ...
         ("is_moving_ego",    is_moving_video(EGO)),
         ("d_min_violated",   d_min_violated(EGO, YIELD_VEHICLE1)),
         ("weather",          weather_checking(weather_condition))],
        [YIELD_VEHICLE1])
    return prop


def long_following_rss_violation_interval_road_surface(road_surface_condition,
                                                        same_lane_switch_frames=2,
                                                        same_lane_hold_frames=1,
                                                        within_dist_hold_frames=1):
    """RSS d_min violation interval property filtered to a specific road surface condition."""
    Q = "within_distance & same_lane & behind & is_moving_ego & d_min_violated"
    ltl = f"({Q}) & (({Q}) U (({Q}) & X(~({Q})))) & road_surface"
    prop = SymbolicProperty(
        f"long_following_rss_violation_interval_{road_surface_condition}",
        ltl,
        [("within_distance", debounced_is_too_close(EGO, YIELD_VEHICLE1, 'visible',
                                                     hold_frames=within_dist_hold_frames)),
         ("same_lane",        nearest_same_lane(YIELD_VEHICLE1, EGO,
         ...
         ("is_moving_ego",    is_moving_video(EGO)),
         ("d_min_violated",   d_min_violated(EGO, YIELD_VEHICLE1)),
         ("road_surface",     road_surface_checking(road_surface_condition))],
        [YIELD_VEHICLE1])
    return prop
...
all_symbolic_properties.append(long_following_rss_violated())
all_symbolic_properties.append(long_following_rss_violation_interval())

for _wc in _WEATHER_CONDITIONS:
    all_symbolic_properties.append(long_following_rss_violation_interval_weather(_wc))

for _rs in _ROAD_SURFACE_CONDITIONS:
    all_symbolic_properties.append(long_following_rss_violation_interval_road_surface(_rs))
