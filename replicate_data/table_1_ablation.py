#!/usr/bin/env python3
"""
table_1_ablation.py — Reproduce Table 1 (ablation study) from the paper.
...
many-to-one interval matching with 50% overlap threshold.
...
def evaluate_scenario(level_extractions: dict, uuids: list[str],
    ...
    return ec.compute_metrics_many_to_one(all_ext, all_gt,
                                          min_coverage=MIN_OVERLAP,
...
def main() -> None:
    ...
    for level_label, sg_dir, track_aware in DATA_CONFIGS:
        ...
        rows.append((level_label, "φ_long_following (30 m)",
                     m30["n"], m30["precision"], m30["recall"], m30["f1"]))

        m60 = evaluate_scenario(level_extractions, uuids,
                                "long_following_visible_3", "lead_vehicle_in_ego_lane_60m", track_aware)
        rows.append((level_label, "φ_long_following (60 m)",
                     m60["n"], m60["precision"], m60["recall"], m60["f1"]))

        mci = evaluate_scenario(level_extractions, uuids,
                                "cut_in_from_right", "cut_in_from_right_full_sequence", track_aware)
        rows.append((level_label, "φ_cut_in",
                     mci["n"], mci["precision"], mci["recall"], mci["f1"]))

    col_level    = 42
    col_scenario = 26
    header = (f"{'SG Level':<{col_level}}  {'Scenario':<{col_scenario}}  "
              f"{'N':>5}  {'Precision':>9}  {'Recall':>6}  {'F1':>6}")
    sep = "-" * len(header)

    print()
    print("Table 1: Ablation — many-to-one matching, 50% overlap, track-aware (rows 1-9) / non-track-aware (rows 10-12)")
    print(sep)
    print(header)
    print(sep)

    prev_level = None
    for level_label, scenario_label, n, precision, recall, f1 in rows:
        ...
        print(f"{level_col:<{col_level}}  {scenario_label:<{col_scenario}}  "
              f"{n:>5}  {precision:>9.3f}  {recall:>6.3f}  {f1:>6.3f}")
